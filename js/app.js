...
(function () {
  ...
  /* ---------- 라우팅 ---------- */
  const TITLES = { dashboard: '관제', games: '경기 탐색', seats: '좌석 현황', alerts: '감시 목록', log: '송출 로그' };
  ...
  /* ---------- 모바일 사이드바 ---------- */
  function openSidebar() { $('#nav-menu').classList.toggle('open'); $('#backdrop').classList.toggle('show'); }
  function closeSidebar() { $('#nav-menu').classList.remove('open'); $('#backdrop').classList.remove('show'); }
  ...
  BA.app.showApp = function () {
    ...
    if (BA.feed) BA.feed.initRadar();   // 레이더 구장 점 생성(최초 1회)
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    BA.auth.init();
    if (BA.console) BA.console.init();   // 로그인 콘솔 연출(부팅/시계/flip/관전모드)
    bindEvents();
